...
For custom subgraphs without a contract address, `subgraph-create` creates project files manually instead of using `ormi-cli init`.
...
- **`subgraph-deploy`, `subgraph-manage`, `subgraph-query`, `subgraph-monitor`**: MCP authentication is required. If `whoami` fails, tell the user to authenticate via MCP and STOP. Only fall back to CLI alternatives if MCP is completely unavailable.
