...
Priority order:
1. Use `ormi-cli init` to create the initial project
2. Use `ormi-cli add` to add new contracts or data sources
3. Use `ormi-cli codegen` immediately after schema or ABI changes
4. Use direct file edits only for the parts `ormi-cli` does not generate correctly or cannot express

Do not start by hand-writing `subgraph.yaml`, `schema.graphql`, or mappings from a blank file if `ormi-cli` can create the baseline first.
...
```bash
ormi-cli add <address> --abi ./abis/OtherContract.json
```
...
First inspect what `ormi-cli init` or `ormi-cli add` already produced. Modify the schema only where the generated version is incomplete or mismatched with the approved plan.
...
First inspect the manifest generated by `ormi-cli`. Prefer targeted edits over rewriting the manifest manually.
...
```bash
ormi-cli codegen   # regenerate if schema or ABI changed
ormi-cli build
```
